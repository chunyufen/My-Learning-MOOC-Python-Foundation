...
"""
dayup = pow(1.001, 365)  # 首先，变量dayup通过pow(1.001, 365)计算了每天进步1%的效果。这里的1.001代表每天增长的比例，而365是一年的天数。这意味着如果每天都比前一天多出1%，那么一年后的增长效果就是dayup的值。
daydown = pow(0.999, 365)  # 接着，变量daydown通过pow(0.999, 365)计算了每天退步1%的效果。这里的0.999代表每天减少的比例，同样地，365是一年的天数。这意味着如果每天都比前一天少了1%，那么一年后的减少效果就是daydown的值。
