...
"""
# 首先，定义了一个字符串weekStr，它包含了按顺序排列的星期一到星期日的中文名称，每个名称之间没有空格。
weekStr = "星期一星期二星期三星期四星期五星期六星期日"
# 然后，代码使用input函数提示用户输入一个数字（1-7），代表星期一到星期日。eval函数用于将用户输入的字符串转换为整数。
weekId = eval(input("请输入星期数字(1-7)："))
# 接下来，计算字符串中对应星期名称的起始位置。由于每个星期的名称由三个中文字符组成，所以通过(weekId - 1) * 3计算出起始位置的索引。这里weekId - 1是因为字符串索引是从0开始的，而星期数字是从1开始的。
pos = (weekId - 1 ) * 3
# 最后，使用字符串切片功能weekStr[pos: pos+3]来获取并打印出从pos位置开始的三个字符，即对应的星期名称。
